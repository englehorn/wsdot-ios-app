//
//  MountainPassAlertDetailViewController.swift
//  WSDOT
//
...
class MountainPassAlertDetailViewController: RefreshViewController, INDLinkLabelDelegate, MapMarkerDelegate, GMSMapViewDelegate {

    var alertId = 0
    var passId = 0
    var mountainPassItem = MountainPassItem()
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        performSegue(withIdentifier: "MountainPassTabView", sender: self)
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        MyAnalytics.screenView(screenName: "PassAlert")
                ...
                if (error == nil) {
                    ...
                    DispatchQueue.main.async { [weak self] in
                        if let selfValue = self {
                            ...
                            if let alertItemValue = MountainPassStore.findPassAlert(withId: selfValue.alertId) {
                                selfValue.passAlertItem = alertItemValue
                                selfValue.displayPassAlert()

                            }

                            else if (MountainPassStore.findPass(withId: selfValue.alertId) == nil) {
                                DispatchQueue.global(qos: DispatchQoS.QoSClass.userInitiated).async { [weak self] in
                                    MountainPassStore.updatePasses(force, completion: { error in
                                        if (error == nil) {
                                            // Reload tableview on UI thread
    ...
    func displayPassAlert() {
        
        title = "Alert"
        
        categoryImage.image = UIHelpers.getPassAlertIcon(forAlert: self.passAlertItem)
        categoryLabel.text = passAlertItem.eventCategoryTypeDescription
        ...
        if (passAlertItem.roadName != "" && passAlertItem.roadDirection != "") {
            location = "<h1>" + passAlertItem.roadName + " " + passAlertItem.roadDirection + "</h1>"
        }
        
        let description = location + "<b>Description: </b>" + passAlertItem.headlineMessage
        ...
        if let alertPubDateValue = try? TimeUtils.formatTimeStamp(passAlertItem.createdDate, dateFormat: "yyyy-MM-dd hh:mm a") {
            alertPubDate = alertPubDateValue
        } else {
            alertPubDate = TimeUtils.parseJSONDateToNSDate(passAlertItem.createdDate)
        }

        if let date = alertPubDate {
            updateTimeLabel.text = TimeUtils.timeAgoSinceDate(date: date, numericDates: false)
        } else {
            updateTimeLabel.text = "unavailable"
        }

        // if location is 0,0 set coordinates in center of WA so we can
        // show the whole state
        let lat = self.passAlertItem.displayLatitude.isEqual(to: 0.0) ? 47.7511 : self.passAlertItem.displayLatitude
        let long = self.passAlertItem.displayLongitude.isEqual(to: 0.0) ? -120.7401 : self.passAlertItem.displayLongitude
        
        let zoom = (self.passAlertItem.displayLatitude.isEqual(to: 0.0) && self.passAlertItem.displayLongitude.isEqual(to: 0.0)) ? 6 : 12
        
        alertMarker.position = CLLocationCoordinate2D(
            latitude: lat,
            longitude: long)
        
        alertMarker.icon = UIHelpers.getPassAlertIcon(forAlert: self.passAlertItem)
    ...
    func mapReady() {
        ...
        if (self.passAlertItem.displayLatitude.isEqual(to: 0.0)
            && self.passAlertItem.displayLongitude.isEqual(to: 0.0)) {
            ...
            if let mapView = embeddedMapViewController.view as? GMSMapView{
                ...
                alertMarker.map = mapView
                mapView.settings.setAllGesturesEnabled(true)
                mapView.moveCamera(GMSCameraUpdate.setTarget(CLLocationCoordinate2D(latitude: self.passAlertItem.displayLatitude, longitude: self.passAlertItem.displayLongitude), zoom: 12))
            }
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "MountainPassTabView" {
            let destinationViewController = segue.destination as! MountainPassTabBarViewController
            destinationViewController.passItem.id = passId
